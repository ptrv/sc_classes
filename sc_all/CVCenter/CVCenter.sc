...
CVCenter {
		...
		if(cvsList.isNil, {
			...
			if(cvs.isNil, {
				...
				if(cvs.isKindOf(Dictionary).not and:{
					...
					"Arguments for CVCenter have to be either a Dictionary, an IdentityDictionary or an Event.".throw;
				}, {
	...
	*setup_ { |mode=0, mean|
		...
		if(Window.allWindows.select({ |w| "^CVCenter\ \-\ MIDI\-Setup".matchRegexp(w.name) == true }).size > 0, {
			...
			cvWidgets.keysValuesDo({ |k, cv| [k, cv].postln; cv.midiSetUp_(mode, mean) })
		})
	}
	...
	*put { |...args|
		...
		inputArgs.pairsDo({ |key, cv|
			if(cv.isKindOf(CV).not, {
				("The value provided for key '"++key.asString++"' doesn't appear to be a CV.\nPlease choose a valid input!").throw;
			});
			if(overwrite, {
	...
	*use { |key, spec, value|
		...
		key ?? { 
			"You can not use a CV in CVCenter without providing a key-name".throw;
		};
